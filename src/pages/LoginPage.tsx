...
export const LoginPage: React.FC<LoginPageProps> = ({ onLogin }) => {
  ...
  const [password, setPassword] = useState('aegis-investigator-pass');
      ...
      <div className="w-full max-w-md bg-[#151E2E] border border-[#263244] rounded-xl shadow-2xl p-6 md:p-8 space-y-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4 font-mono text-xs">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full flex items-center justify-center gap-2 py-2.5 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white rounded-md text-xs font-semibold uppercase tracking-wider transition shadow-lg mt-2"
          >
            <span>{loading ? 'Authenticating...' : 'Access Forensic Workstation'}</span>
